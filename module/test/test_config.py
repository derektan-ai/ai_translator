...
class TestConfig(unittest.TestCase):
    ...
    def test_module_import_directory_creation(self):
        """测试模块导入时的目录创建逻辑，覆盖第116行注释"""
        # 临时修改sys.modules，强制重新导入模块
        import importlib
        import sys
        
        # 保存原始模块引用
        original_module = sys.modules.get('module.config')
        
        try:
            # 从sys.modules中移除模块，强制重新导入
            if 'module.config' in sys.modules:
                del sys.modules['module.config']
            
            # 重新导入模块，这将执行包括第116行注释在内的所有顶层代码
            import module.config
            
            # 验证目录创建逻辑正确执行
            self.assertTrue(os.path.exists(module.config.Config.LOG_DIR))
            self.assertTrue(os.path.exists(module.config.Config.RESULT_DIR))
            
            # 验证目录路径正确设置
            self.assertIsNotNone(module.config.Config.LOG_DIR)
            self.assertIsNotNone(module.config.Config.RESULT_DIR)
            
        finally:
            # 恢复原始模块引用
            if original_module:
                sys.modules['module.config'] = original_module
