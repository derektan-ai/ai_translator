...
class TestResultRecorder(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_result_dir = os.path.join(os.path.dirname(__file__), 'test_result')
        Config.RESULT_DIR = self.test_result_dir
    ...
    def tearDown(self):
        ...
        if os.path.exists(self.test_result_dir):
            for file in os.listdir(self.test_result_dir):
                file_path = os.path.join(self.test_result_dir, file)
    ...
    def test_get_file_path(self, mock_create_file):
        """测试获取文件路径方法"""
        # Set the return value to the correct test result path
        mock_create_file.return_value = os.path.join(self.test_result_dir, 'test_file.txt')
...
"""
    ...
    def test_convert_result_format_all_failed(self, mock_msgbox, mock_listdir, mock_exists, mock_convert):
        ...
        # 避免实际创建文件
        with patch('module.result_recorder.ResultRecorder._create_result_file') as mock_create_file, \
             patch('module.result_recorder.ResultRecorder._write_header'):
            mock_create_file.return_value = os.path.join(self.test_result_dir, 'test_file.txt')
            recorder = ResultRecorder('en', 'zh', self.mock_logger)
            # 使用正确的格式选项，键值对
    ...
    def test_report_result_status_output_without_logger(self):
        ...
        with patch('module.result_recorder.ResultRecorder._create_result_file') as mock_create_file:
            ...
            with patch('module.result_recorder.os.path.exists', return_value=True), \
                 ...
                 patch('module.result_recorder.os.path.basename', return_value=os.path.join(self.test_result_dir, 'test_file.txt')), \
                 patch('builtins.print') as mock_print:
    ...
    def test_report_result_status_with_translations(self):
        ...
        with patch('module.result_recorder.ResultRecorder._create_result_file') as mock_create_file, \
            ...
            # 验证logger记录了两个输出信息
            expected_calls = [
                call(f"{LANGUAGE_LABELS['zh'].translation_saved_to}: {os.path.join(self.test_result_dir, 'test_file.txt')}"),
