...
class ControlWidgets:
    ...
    font_size_label: None = None
    font_size_slider: None = None
    font_size_value: None = None
...
class UI(QWidget):
    ...
    def _draw_text(self, painter):
        ...
        # 设置字体及相关参数 - 使用滑动条控制的字体大小
        font_size = (
            self.widgets.font_size_slider.value()
            if hasattr(self.widgets, 'font_size_slider')
            else 12
        )
        font = self.get_font(font_size, QFont.Bold)
    ...
    def initUI(self):
        ...
        self.widgets.control_layout.addWidget(self.widgets.font_size_label)
        self.widgets.control_layout.addWidget(self.widgets.font_size_slider)
        self.widgets.control_layout.addWidget(self.widgets.font_size_value)
        ...
        # 设置关闭按钮事件处理
        self._setup_close_button_events()
    ...
    def create_control_widgets(self):
        ...
        self._create_font_size_controls()
    ...
    def _create_font_size_controls(self):
        """创建字体大小调整控件"""
        # 字体大小标签
        self.widgets.font_size_label = QLabel(INFO.get("font_size_label", self.language))
        self.widgets.font_size_label.setFont(self.get_font(9, QFont.Bold))
        self.widgets.font_size_label.setStyleSheet("color: white; background-color: transparent;")
        self.widgets.font_size_label.setAlignment(Qt.AlignVCenter)

        # 字体大小滑动条
        self.widgets.font_size_slider = QSlider(Qt.Horizontal)
        self.widgets.font_size_slider.setMinimum(8)
        self.widgets.font_size_slider.setMaximum(24)
        self.widgets.font_size_slider.setValue(12)  # 默认字体大小
        self.widgets.font_size_slider.setFixedWidth(100)
        self.widgets.font_size_slider.setStyleSheet("""
        ...
        # 字体大小数值显示
        self.widgets.font_size_value = QLabel("12")
        self.widgets.font_size_value.setFont(self.get_font(9))
        self.widgets.font_size_value.setStyleSheet("color: white; background-color: transparent;")
        self.widgets.font_size_value.setAlignment(Qt.AlignCenter)
        self.widgets.font_size_value.setFixedWidth(30)

        # 连接滑动条信号
        self.widgets.font_size_slider.valueChanged.connect(self.on_font_size_changed)

    def on_font_size_changed(self, value):
        """字体大小变化时的处理函数"""
        # 更新数值显示
        self.widgets.font_size_value.setText(str(value))
        # 重新绘制字幕区域
        self.update()

    def _setup_close_button_events(self):
        """设置关闭按钮的事件处理"""
        # 为关闭按钮添加透明度变化效果和光标处理
    ...
    def get_font(self, size=9, weight=QFont.Normal):
        ...
        unified_font_family = "Microsoft YaHei, SimHei, Heiti TC, SimSun, NSimSun, Arial Unicode MS, sans-serif"
        font = QFont(unified_font_family, size, weight)
        return font
